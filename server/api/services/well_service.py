...
from api.models import Inclinometry, Mer, Rate, WellCase, WellPerforation, WellPump, WellHorizon
from api.serializers import IncSerializer, WellCaseSerializer, WellPerforationSerializer, \
    WellPumpSerializer, WellHorizonSerializer
# GET CHILD OBJECTS
...
# /wells/{id}/zones GET
def get_well_horizons(pk):
    horizons = WellHorizon.objects.filter(well_id=pk)
    serializer = WellHorizonSerializer(horizons, many=True)
    return Response(serializer.data)
...
def delete_well_horizons(pk):
    count = WellHorizon.objects.filter(well_id=pk).delete()
